...
class MicrosoftIntegration:
    async def verify(self, credential: IntegrationCredential) -> tuple[bool, str]:
        if not app_settings.get("ms_app_client_id") or not app_settings.get("ms_app_client_secret"):
            return False, "MS_APP_CLIENT_ID/MS_APP_CLIENT_SECRET ej konfigurerade"
        if not credential.tenant_id:
